document.addEventListener('DOMContentLoaded', () => {
    ...
    function addSourceHoverTooltips(container) {
        ...
        citations.forEach((citation, index) => {
            // Skip if this citation already has a tooltip
            if (citation.hasAttribute('data-tooltip-added')) {
                return;
            }
            ...
            // Add source citation class if not already present
            if (!citation.classList.contains('source-citation')) {
                citation.classList.add('source-citation');
            }
            
            // Extract information for tooltip
            let domain = '';
            let title = '';
            let url = '';
            ...
            // Store reference to tooltip on the citation
        ...
        console.log(`Successfully added hover tooltips to ${citations.length} citations`);
    }
    ...
    // Voice mode functionality - Enhanced transcription quality
    let isVoiceModeActive = false;
    let recognition = null;
    let isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
    let transcriptBuffer = ''; // Buffer for building better transcripts
    let lastFinalTranscript = ''; // Track last final result to avoid duplicates
    let confidenceThreshold = 0.7; // Minimum confidence for accepting results
    let silenceTimer = null; // Timer for detecting pauses
    ...
    // Enhanced text processing for better transcription
    function processTranscript(text, confidence = 1.0) {
        ...
        if (confidence < confidenceThreshold) {
            console.log(`Low confidence transcript (${confidence.toFixed(2)}): "${processed}"`);
            return processed; // Still return but mark as low confidence
        }
        
        // Smart capitalization
        processed = smartCapitalization(processed);
    ...
    // Smart capitalization for better readability
    function smartCapitalization(text) {
        if (!text) return '';
        ...
        const properNouns = ['google', 'microsoft', 'apple', 'amazon', 'facebook', 'twitter', 'linkedin', 'youtube', 'netflix', 'tesla', 'ai', 'api', 'cpu', 'gpu', 'ram', 'ssd', 'html', 'css', 'javascript', 'python', 'java', 'sql', 'json', 'xml', 'http', 'https', 'www', 'url', 'ip', 'tcp', 'udp', 'dns'];
        
        properNouns.forEach(noun => {
            const regex = new RegExp(`\\b${noun}\\b`, 'gi');
            text = text.replace(regex, (match) => {
                if (noun.length <= 3) {
                    return match.toUpperCase(); // Acronyms in all caps
                } else {
                    return match.charAt(0).toUpperCase() + match.slice(1).toLowerCase(); // Proper nouns
                }
            });
        });
    ...
    // Add intelligent punctuation based on speech patterns
    function addIntelligentPunctuation(text) {
        ...
        const startsWithQuestion = questionWords.some(word => 
            text.toLowerCase().startsWith(word + ' ') || text.toLowerCase() === word
        );
        
        // Command indicators
        const commandWords = ['show', 'tell', 'explain', 'describe', 'find', 'search', 'get', 'give', 'help', 'create', 'make', 'generate'];
        ...
        if (!text.match(/[.!?]$/)) {
            if (startsWithQuestion) {
                text += '?';
            } else if (isCommand || text.toLowerCase().includes('please')) {
                text += '.'; // Commands typically end with periods
            } else {
                text += '.'; // Default to period
            }
        }
    ...
    function detectOptimalLanguage() {
        ...
        // Map to supported speech recognition languages with regional variants
        const langMap = {
            'en': 'en-US',
            'es': 'es-ES',
            'fr': 'fr-FR',
        ...
        return langMap[langCode] || userLang;
    }
    ...
    function initSpeechRecognition() {
        ...
        try {
            ...
            recognition.lang = detectOptimalLanguage(); // Use detected optimal language
            
            // Enhanced audio processing (if supported)
            if ('serviceURI' in recognition) {
                recognition.serviceURI = 'wss://api.speechly.com/ws/v1'; // Alternative service if available
            }
            ...
            recognition.onstart = () => {
                ...
                // Add visual feedback for active listening
                searchInput.style.borderColor = '#4CAF50';
                searchInput.style.boxShadow = '0 0 5px rgba(76, 175, 80, 0.3)';
            };

            recognition.onresult = (event) => {
                console.log('Enhanced speech recognition result received');
                ...
                for (let i = 0; i < event.results.length; i++) {
                    ...
                    if (result.isFinal) {
                        ...
                        let bestConfidence = result[0].confidence || 0.8; // Default confidence
                        
                        // Check all alternatives for the best one
                        ...
                        const processedText = processTranscript(bestResult.transcript, bestConfidence);
                        
                        // Avoid duplicate final results
                        if (processedText && processedText !== lastFinalTranscript) {
                            currentFinalTranscript += processedText;
                            lastFinalTranscript = processedText;
                            console.log(`Final transcript (confidence: ${bestConfidence.toFixed(2)}): "${processedText}"`);
                        }
                    } else {
                        // Handle interim results with confidence filtering
                        const transcript = result[0].transcript;
                        const confidence = result[0].confidence || 0.5;
                        
                        if (confidence > 0.3) { // Lower threshold for interim results
                            currentInterimTranscript += processTranscript(transcript, confidence);
                ...
                // Update the input with enhanced logic
                if (currentFinalTranscript) {
                    // Add final transcript with proper spacing
                    const separator = currentValue && !currentValue.endsWith(' ') ? ' ' : '';
                    searchInput.value = currentValue + separator + currentFinalTranscript;
                    transcriptBuffer = searchInput.value;
                    
                    // Provide haptic feedback on mobile for final results
                    ...
                    const separator = currentValue && !currentValue.endsWith(' ') ? ' ' : '';
                    searchInput.value = transcriptBuffer + separator + currentInterimTranscript;
                    
                    // Add visual feedback for interim results
                    searchInput.style.fontStyle = 'italic';
                    searchInput.style.opacity = '0.8';
                    
                    // Update placeholder with confidence indicator
                    if (highestConfidence > 0.7) {
                        searchInput.placeholder = '🎤 Listening... (High confidence)';
                    } else if (highestConfidence > 0.4) {
                        searchInput.placeholder = '🎤 Listening... (Medium confidence)';
                    } else {
                        searchInput.placeholder = '🎤 Listening... (Low confidence - speak clearer)';
                    }
                } else {
                    // Reset to final transcript if no interim results
                    searchInput.value = transcriptBuffer;
                ...
                // Set new silence timer for auto-stop after inactivity
                silenceTimer = setTimeout(() => {
                    if (isVoiceModeActive && transcriptBuffer.trim()) {
            ...
            recognition.onerror = (event) => {
                ...
                if (event.error === 'not-allowed' || event.error === 'service-not-allowed') {
                    ...
                    searchInput.placeholder = '🎤 No speech detected - try speaking again';
                    if (isVoiceModeActive) {
                        // Don't restart immediately, let the silence timer handle it
                        return;
                    }
                } else if (event.error === 'aborted') {
                    console.log('Speech recognition aborted by user');
                    searchInput.placeholder = 'Voice recognition stopped';
                    return;
                } else if (event.error === 'network') {
                    searchInput.placeholder = '❌ Network error - check connection';
                    alert('Network error during speech recognition. Please check your internet connection.');
                } else if (event.error === 'audio-capture') {
                    searchInput.placeholder = '❌ Audio capture error';
                    alert('Audio capture error. Please check your microphone.');
                } else {
                    searchInput.placeholder = `❌ Recognition error: ${event.error}`;
                }
                    ...
                    setTimeout(() => {
                        if (isVoiceModeActive) {
                            try {
                                ...
                                console.error('Error restarting enhanced recognition:', error);
                                stopVoiceMode();
                            }
                        }
            ...
            // Add speech start detection for better UX
            recognition.onspeechstart = () => {
                console.log('Speech detected');
                const searchInput = document.getElementById('search-input');
                searchInput.placeholder = '🎤 Processing speech...';
                searchInput.style.borderColor = '#2196F3';
                searchInput.style.boxShadow = '0 0 5px rgba(33, 150, 243, 0.5)';
            };
    ...
    // Stop voice mode with enhanced cleanup
    function stopVoiceMode() {
        ...
        // Clean up transcript buffers
        if (transcriptBuffer && searchInput.value !== transcriptBuffer) {
            // If there's a pending transcript, apply it
            searchInput.value = transcriptBuffer;
        }
        ...
        // Auto-resize textarea one final time
        searchInput.style.height = 'auto';
        searchInput.style.height = (searchInput.scrollHeight) + 'px';
        ...
        if (recognition) {
            try {
                recognition.stop();
                console.log('Enhanced speech recognition stopped and cleaned up');
            } catch (error) {
    ...
    // Start voice mode with enhanced audio settings
    async function startVoiceMode() {
        try {
            console.log('Starting enhanced voice mode on mobile:', isMobile);
            ...
            try {
                ...
                // Analyze audio capabilities for better setup
                ...
                if (audioTracks.length > 0) {
                    ...
                    // Apply enhanced audio settings if supported
                    const constraints = {};
                    ...
                    if (Object.keys(constraints).length > 0) {
                        try {
                            ...
                            console.log('Enhanced audio constraints applied:', constraints);
                        } catch (e) {
                            console.warn('Could not apply enhanced audio constraints:', e);
                        }
                    }
                }
                
                // Stop the stream immediately after getting permission and setup
                stream.getTracks().forEach(track => track.stop());
                console.log('Enhanced microphone permission granted with audio optimization');
            } catch (permError) {
                console.error('Microphone permission denied:', permError);
                alert('Please allow microphone access to use voice mode. For best results, ensure you\'re in a quiet environment.');
                return;
            }

            // Initialize speech recognition if not already done
            if (!recognition && !initSpeechRecognition()) {
                alert('Speech recognition is not supported in your browser. Please use Chrome, Safari, or Edge for best results.');
                return;
            }

            // Reset all transcript tracking
            transcriptBuffer = '';
            lastFinalTranscript = '';
            ...
            try {
                ...
                // Provide user guidance for better transcription
                const searchInput = document.getElementById('search-input');
                searchInput.placeholder = '🎤 Ready to listen... Speak clearly and naturally!';
                
            } catch (startError) {
                console.error('Error starting enhanced speech recognition:', startError);
                
                // Handle specific errors with better user feedback
                if (startError.message && startError.message.includes('already started')) {
                    console.log('Speech recognition already active, restarting...');
                    try {
                        recognition.stop();
                        setTimeout(() => {
                            if (isVoiceModeActive) {
                        ...
                        }, 500); // Longer delay for enhanced mode
                    } catch (e) {
                        console.error('Failed to restart enhanced recognition:', e);
                        stopVoiceMode();
                        alert('Voice recognition restart failed. Please try again in a moment.');
                    }
                } else {
                    ...
                    if (initSpeechRecognition()) {
                        try {
                            ...
                            console.error('Enhanced recognition retry failed:', retryError);
                            stopVoiceMode();
                            alert('Unable to start enhanced voice recognition. Please check your microphone and try again.');
                        }
                    } else {
                        stopVoiceMode();
                        alert('Voice recognition initialization failed. Please ensure you have a working microphone.');
                    }
                }
            }
        } catch (error) {
            console.error('Error in enhanced startVoiceMode:', error);
            stopVoiceMode();
            alert('Enhanced voice mode failed to start. Please check your browser compatibility and microphone settings.');
        }
    }
